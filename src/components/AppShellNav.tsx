...
import { isAdmin, isPlayer, roleLabel } from "@/lib/roles";

type NavItem = {
  href: string;
  label: string;
  icon: React.ReactNode;
  // If set, this item is only shown to users of that role bucket.
  showFor?: "organizer" | "player" | "both" | "admin";
};
  ...
  },
  {
    ...
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
        ...
        <path
          d="M3 19a6 6 0 0 1 12 0M14 19a5 5 0 0 1 8 0"
          stroke="currentColor"
          strokeWidth="2"
        />
      </svg>
  ...
  },
  {
    ...
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
        ...
        <path
          d="M5 12c3-3 11-3 14 0M5 12c3 3 11 3 14 0"
          stroke="currentColor"
          strokeWidth="2"
        />
      </svg>
...
export function AppShellNav() {
  ...
  const player = isPlayer(role);
  const admin = isAdmin(role);

  const items = NAV.filter((item) => {
    if (!item.showFor || item.showFor === "both") return true;
    if (item.showFor === "player") return player;
    if (item.showFor === "organizer") return !player;
    if (item.showFor === "admin") return admin;
    return true;
  });
